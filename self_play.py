...
class SelfPlay:
    ...
    def continuous_self_play(
        self, shared_storage, replay_buffer, test_mode=False
    ):
        while ray.get(
    ...
    def play_game(
        ...
        with torch.no_grad():
            while (
                not done
                and len(game_history.action_history) <= self.config.max_moves
            ):
                assert (
                    len(numpy.array(observation).shape) == 3
                ), f"Observation should be 3 dimensionnal instead of {len(numpy.array(observation).shape)} dimensionnal. Got observation of shape: {numpy.array(observation).shape}"
                assert (
                    numpy.array(observation).shape
                ...
                if opponent == "self" or muzero_player == self.game.to_play():
                    ...
                    action = self.select_action(
                        ...
                        or len(game_history.action_history)
                        < temperature_threshold
                        else 0,
                    )
...
class MCTS:
    ...
    def run(
        ...
        if override_root_with:
            ...
            root.expand(
                legal_actions,  # [julen] in the root only the legal actions are expanded
                to_play,
                reward,
                policy_logits,
                hidden_state,
    ...
    def add_exploration_noise(self, dirichlet_alpha, exploration_fraction):
        ...
        for a, n in zip(actions, noise):
            self.children[a].prior = (
                self.children[a].prior * (1 - frac) + n * frac
            )
